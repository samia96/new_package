...
class ColumnsComponent extends NestedComponent_1.default {
    ...
    init() {
        super.init();
        this.columns = [];
        lodash_1.default.each(this.component.columns, (column, index) => {
            this.columns[index] = [];
            if (!column.size) {
                column.size = 'md';
            }
    ...
    justifyColumn(items, index) {
        const toAdjust = lodash_1.default.every(items, item => !item.visible);
        const column = this.component.columns[index];
        const width = (toAdjust && items.length) ? 0 : column.width;
        const shouldRedraw = !lodash_1.default.isEqual(width, column.currentWidth);
